...
def _connect():
    try:
        ...
        _data = {'grant_type': "authorization_code",
                 "code": request.form["code"], "redirect_uri": "https://front-service-dot-project-h-283916.ew.r.appspot.com/spotify_redirect", "client_id": client["client_id"], "client_secret": client["client_secret"]}
        ...
        ref.update({"spotify": {"refresh_token": _dict["refresh_token"], "access_token": _dict["access_token"],
                                "expires_at": datetime.datetime.utcnow() + datetime.timedelta(seconds=exp_in)}})
...
def _playlists():
    headers = {'Authorization': 'Bearer ' + g.user.spotify["access_token"]}
    r = requests.get(
        'https://api.spotify.com/v1/me/playlists?offset=0&limit=50', headers=headers)
    if r.status_code:
        return r.json(), 200
    return {"status": "error"}, 400

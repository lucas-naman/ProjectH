...
def _connect():
    try:
        ...
        _data = {'app_id': client['client_id'],
                 "secret": client["client_secret"], "code": request.form["code"]}
        r = requests.get(
            "https://connect.deezer.com/oauth/access_token.php", _data)
        ref = db.collection(u'Users').document(g.user.id)
        ref.update(
            {"deezer": {"token": r.text[r.text.index('=') + 1:r.text.index('&')]}})
...
def _info():
    r = requests.get(
        'https://api.deezer.com/user/me?access_token=' + g.user.deezer["token"])
...
def _playlists():
    r = requests.get(
        'https://api.deezer.com/user/me/playlists?access_token=' + g.user.deezer["token"])
    if r.status_code:
        return r.json(), 200
    return {"status": "error"}, 400
...
@deezer.route("/disconnect", methods=['DELETE'])
@check_token
def _disconnect():
    ref = db.collection(u'Users').document(g.user.id)
    ref.update({"deezer": firestore.DELETE_FIELD})
    return jsonify({"status": "ok"})
